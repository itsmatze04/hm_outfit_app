...
# einfache Definition „neutraler“ Farben (für Farblogik)
...
# Hilfsfunktionen
# ---------------------------------------------------------
def get_image_path(article_id_str):
    folder = article_id_str[:3]
    filename = "%s.jpg" % article_id_str
...
def compute_color_score(base_row, candidate_row):
    """
    Einfache Heuristik für Farbübereinstimmung.
    Höherer Score = besser passend.
    """
    score = 0.0
...
def rerank_by_color(base_row, df_cat):
    """
    Ergänzt eine Spalte 'color_score' und sortiert:
    zuerst nach color_score (absteigend), dann nach copurchase_count (absteigend).
    """
    if df_cat.empty:
        return df_cat
...
def get_outfit_recommendations(base_article_id, df_articles, df_cop, n_per_category=3):
    ...
    - Farblogik: Kandidaten per color_score + copurchase_count sortieren
...
# ---------------------------------------------------------
def main():
    st.title("Deine Mutter würde diesen Outfit Recommender empfehlen (hat sie gestern abend erzählt)")

    # Daten robust laden
    try:
        df_articles = load_articles()
    except FileNotFoundError:
        st.error("articles_filtered.csv nicht gefunden. Pfad prüfen: %s" % ARTICLES_FILE)
        st.stop()
    except Exception as e:
        st.error("Fehler beim Laden von articles_filtered.csv: %s" % e)
        st.stop()

    try:
        df_cop = load_copurchase()
    except FileNotFoundError:
        st.error("copurchase_filtered.csv nicht gefunden. Pfad prüfen: %s" % COPURCHASE_FILE)
        st.stop()
    except Exception as e:
        st.error("Fehler beim Laden von copurchase_filtered.csv: %s" % e)
        st.stop()
    ...
    with col_left:
        ...
        st.markdown("**ID:** %s" % base_row["article_id_str"])
        st.markdown("**Name:** %s" % base_row.get("prod_name", ""))
            ...
            for macro in MACRO_DISPLAY_ORDER:
                ...
                for col, (_, row) in zip(cols, rec_df.iterrows()):
                    with col:
                        ...
                        st.caption("ID: %s" % row["article_id_str"])

                        cps = int(row.get("copurchase_count", 0))
                        cs = float(row.get("color_score", 0.0))
                        st.caption("Co-Purchase Count: %d · Color-Score: %.1f" % (cps, cs))
