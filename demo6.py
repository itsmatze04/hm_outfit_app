...
# ---------------------------------------------------------
# Auswahl-Flow für Basisteil (Buttons + Unterkategorie + Bilder)
# ---------------------------------------------------------
...
def select_base_macro() -> str:
    """
    Schritt 1: Makrokategorie wählen (per Buttons).
    ...
    cols = st.columns(2)  # 2 Spalten in der Sidebar
    clicked_macro = None
...
def select_base_article(df_articles: pd.DataFrame, base_macro: str):
    """
    Schritt 2 & 3 (komplett in der Sidebar):
    - Unterkategorie (product_type_name) innerhalb der Makrokategorie wählen (Buttons)
    - Alle Artikel der gewählten Unterkategorie als Karten (kleines Bild + Button)
    ...
    # ---------------------------------------------
    # Schritt 3: Basisteil innerhalb der Unterkategorie wählen
    # ---------------------------------------------
    st.markdown("**3. Schritt: Basisteil wählen**")
    ...
    for _, row in df_show.iterrows():
        ...
        st.caption(
            f"{row.get('product_type_name', '')} · {row.get('colour_group_name', '')}"
        )
...
def main():
    ...
    # Auswahl-Flow komplett in die Seitenleiste legen
    with st.sidebar.expander("Basisteil auswählen", expanded=True):
        st.subheader("Basisteil")

        # Schritt 1: Makrokategorie wählen (Buttons)
        base_macro = select_base_macro()

        # Schritt 2 & 3: Unterkategorie + konkretes Basisteil wählen
        selected_article_id, base_row = select_base_article(df_articles, base_macro)

    if base_row is None or selected_article_id is None:
        st.info("Bitte in der Seitenleiste ein Basisteil auswählen.")
        st.stop()

    # Hauptlayout: links Basisteil groß, rechts Outfit
    ...
    with col_left:
        ...
        st.caption(f"Gewählte Basiskategorie: {pretty_macro}")
