...
st.set_page_config(
    page_title="H&M Outfit Recommender",
    layout="wide",
    initial_sidebar_state="collapsed",
)

# CSS: Bilder & Buttons wie in der Kumo-Version
st.markdown(
    """
    <style>
        /* Bilder auf exakte Höhe bringen */
...
def assign_gender(idx_name):
    """Leitet Gender (Damen / Herren / Kinder) aus index_name ab."""
    idx = str(idx_name)
    if "Menswear" in idx:
        return "Herren"
...
def assign_style(row):
    """
    Weist einen groben Stil zu: Casual / Business / Sport / Kids.
    Nutzt index_name, product_type_name und prod_name.
    """
    ...
    if pt in BUSINESS_TYPES:
        # Divided / sehr casual Namen -> eher Casual statt Business
...
def load_articles():
    ...
    # Makrokategorie aus product_type_name
    ...
    # Farb-Familien (deine Mapping-Logik aus demo7)
...
def render_select_view(df_articles: pd.DataFrame):
    """
    Landing Page im Stil von demo_np:
    - Buttons: Damen / Herren / Kinder
    - Dropdowns: Stil, Kategorie (macro_category), Artikelart, Farbe (colour_family)
    - Grid: Artikel mit Bild + Button -> setzt base_article_id und wechselt in Outfit-View
    """
    st.title("H&M Outfit Recommender")
    ...
    # ---------------------------------------------
    # 1. GENDER (Buttons Damen / Herren / Kinder)
    # ---------------------------------------------
    ...
    for i, g in enumerate(genders):
        ...
        with col:
            if st.button(f"👤 {g}", key=f"gender_{g}", type=btn_type, use_container_width=True):
                ...
                # Bei Genderwechsel Stil & Kategorie zurücksetzen
    ...
    # ---------------------------------------------
    # 2. FILTERLEISTE: Stil, Kategorie, Artikelart, Farbe
    # ---------------------------------------------
    with st.container(border=True):
        ...
        with fc1:
            ...
            if is_kids:
                ...
                style_opts = ["Casual", "Business / Chic", "Sport"]
        ...
        # 2.2 Kategorie (macro_category -> Dropdown mit MACRO_LABEL_DE)
        with fc2:
        ...
        # 2.3 Artikelart (product_type_name) – nur Unterkategorien der gewählten Makrokategorie
        with fc3:
        ...
        # 2.4 Farbe (colour_family) – aus deiner Farb-Familien-Logik
        with fc4:
    ...
    for i, row in enumerate(df_show.itertuples()):
        with cols[i % 6]:
            ...
            if st.button(label, key=f"btn_{row.article_id}"):
...
def main():
    ...
    # Routing: Landing (Auswahl) vs. Outfit-View
