...
def get_smart_recommendations(base_id, df, df_cop, n=20):
    ...
    if base_id != 999999:
        ...
        if not df_pairs.empty:
            df_pairs["partner_id"] = df_pairs.apply(lambda r: r["article_id_2"] if r["article_id_1"] == base_id else r["article_id_1"], axis=1)
            candidates = df_pairs.merge(df, left_on="partner_id", right_on="article_id")
    
    outfit = {}
    target_macros = [m for m in MACRO_DISPLAY_ORDER if m != base_macro]
    ...
    for target in target_macros:
        ...
        top_picks = final_df.sort_values("match_score", ascending=False).head(10).reset_index(drop=True)
...
def render_landing_page(df):
    ...
    for i, g in enumerate(genders):
        ...
        if [c1, c2, c3][i].button(f"👤 {g}", key=f"g_{g}", type="primary" if isActive else "secondary", use_container_width=True):
    ...
    with st.expander("🌤️ Wetter-Check", expanded=False):
        ...
        if city:
            ...
            if wdata:
                ...
                cond_map = {"Cold": "❄️ Kalt", "Hot": "☀️ Heiß", "Rain": "🌧️ Regen", "Snow": "🌨️ Schnee", "Normal": "🌤️ Normal"}
    ...
    with st.expander("📸 Match my Closet", expanded=False):
        ...
        if uploaded_file:
            ...
            def_idx = 0
            if col_name in col_families: def_idx = col_families.index(col_name)
            ...
            if st.button("Outfit dazu suchen", type="primary"):
                fake_id = 999999
                fake_row = pd.Series({
                    "article_id": fake_id, "article_id_str": "0000000000", "prod_name": "Dein Upload",
                    "product_type_name": "Upload", "macro_category": internal_macro,
                    "colour_group_name": final_col, "colour_family": final_col,
                    "style": style_manual, "gender": st.session_state.sel_gender, "index_name": "Upload"
                })
    ...
    with st.container(border=True):
        ...
        with fc1:
            ...
            if st.session_state.sel_style not in style_opts: st.session_state.sel_style = style_opts[0]
            st.session_state.sel_style = st.selectbox("1. Stil", style_opts, key="sb_style", disabled=is_kids)
        df_s = df_g[df_g["style"] == st.session_state.sel_style]

        with fc2:
            macros = sorted(df_s["macro_category"].dropna().unique())
            if not macros: st.warning("Keine Artikel"); return
            if st.session_state.sel_macro not in macros: st.session_state.sel_macro = "TOP" if "TOP" in macros else macros[0]
            st.session_state.sel_macro = st.selectbox("2. Kategorie", macros, index=macros.index(st.session_state.sel_macro), format_func=lambda x: MACRO_LABEL_DE.get(x, x), key="sb_macro")
        ...
        if st.session_state.sel_subcat != "Alle": df_m = df_m[df_m["product_type_name"] == st.session_state.sel_subcat]

        with fc4:
            colors = ["Alle"] + sorted(df_m["colour_family"].unique())
            st.session_state.sel_colour = st.selectbox("4. Farbe", colors, key="sb_col")
        if st.session_state.sel_colour != "Alle": df_m = df_m[df_m["colour_family"] == st.session_state.sel_colour]

    st.markdown("---")
    if df_m.empty: st.warning("Keine Artikel gefunden."); return

    df_show = df_m.head(48)
    cols = st.columns(6)
    for i, row in enumerate(df_show.itertuples()):
        with cols[i % 6]:
            ...
            if len(clean_name) > 16: clean_name = clean_name[:14] + ".."
            label = f"{clean_name}\n({row.colour_group_name})"
            st.button(label, key=f"btn_{row.article_id}", on_click=set_base_article, args=(row.article_id,), use_container_width=True)
...
def render_outfit_view(df, df_cop, base_id):
    ...
    base_row = get_base_article_row(df, base_id)
    if base_row is None: st.error("Datenfehler"); return

    recommendations = get_smart_recommendations(base_id, df, df_cop, n=10)
